 ...
 * @author SURESH
 */
@WebServlet(name="quizlists", urlPatterns={"/quizlists"})
public class quizlists extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try {
          String c="",cpp="",java="",sql="",datastructure="";
            ...
            String q="select count(ques) from question where quiztype='java';";
            ResultSet rs=s.executeQuery(q);
            if(rs.next()==true)
            {
                java=Integer.toString(rs.getInt(1));
            }
            q="select count(ques) from question where quiztype='c';";
           rs=s.executeQuery(q);
            if(rs.next()==true)
            {
...
q="select count(ques) from question where quiztype='sql';";
             rs=s.executeQuery(q);
            if(rs.next()==true)
            {
               sql=Integer.toString(rs.getInt(1));
            }
             q="select count(ques) from question where quiztype='datastructure';";
          rs=s.executeQuery(q);
            if(rs.next()==true)
            {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet quizlists</title>");  
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet quizlists at " + request.getContextPath () + "</h1>");
            out.println("</body>");
            out.println("</html>");
            */
        }
